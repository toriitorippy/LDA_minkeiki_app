...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        const path_id = BACKEND_URL + '/getid_m/' + item._index
        axios.get(path_id)
            .then(response => {
      ...
      getDataTopic () {
        const path = BACKEND_URL +  '/getpermonth/'+ this.total_num +'/' + this.num
        axios.get(path)
            .then(response => {
